 ...
 *
 */
public class Bank extends MerchantContainer
{
    private BankHeader  Header;
    private BankTrailer Trailer;

    protected Bank(PTIFile f, BankHeader bh)
    {
        super(f);
        Header = bh;
        RecordCount++;
    ...
    {
        if (!t.getClass().getName().equals("com.bottinifuel.chase.PTI.records.BankTrailer"))
            throw new FileFormatException("Invalid trailer record type: expected ST, got " +
                                          t.getClass().getName());
        Trailer = (BankTrailer)t;
        RecordCount++;
    }
    ...
    {
        if (Trailer == null)
            throw new FileFormatException("No ST seen for bank starting on line #" + Header.SeqNum);

        if (RecordCount != Trailer.RecordCount)
            throw new FileFormatException("Bank record count mismatch: BT=" + Trailer.RecordCount + " Saw=" + RecordCount);
        Parent.IncrRecordCount(RecordCount);
        
        if (Merchants.size() != Trailer.MerchantCount)
            throw new FileFormatException("Bank Merchant count mismatch: BT=" + Trailer.MerchantCount + " Saw=" + Merchants.size());
